...
const createInventoryController = async (req, res) => {
  try {
    ...
    if (req.body.inventoryType == "out") {
      ...
      console.log("avail", availableQuantityofBloodGroup);
      console.log("requested", requestedBloodGroup, requestedQuantity);
      //validation
      if (availableQuantityofBloodGroup < requestedQuantity) {
        return res.status(500).send({
          success: false,
          message: `Only ${availableQuantityofBloodGroup} ml of ${requestedBloodGroup.toUpperCase()} Available`,
        });
      }
...
const getOrganisationController = async (req, res) => {
  try {
    ...
    return res.status(200).send({
      success: true,
      message: "Org data fetched succesfully",
      organisations,
    });
  } catch (error) {
    console.log(error);
...
const getOrganisationForHospitalController = async (req, res) => {
  try {
    ...
    return res.status(200).send({
      success: true,
      message: "Hospital Org data fetched succesfully",
      organisations,
    });
  } catch (error) {
    console.log(error);
